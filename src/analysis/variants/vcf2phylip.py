    ...
    with opener(vcf_file, "rt") as vcf:
        for line in vcf:
            ...
            if line.startswith("#CHROM"):
                record = line.split("\t")
                sample_names = [
                    record[i].replace("./", "") for i in range(9, len(record))
                ]
                break
    return sample_names
...
def main():
    ...
    with opener(filename, "rt") as vcf:
                        ...
                        if num_samples_locus < min_samples_locus:
                            ...
                            if is_snp(record):
                                ...
                                if fasta or nexus or not phylipdisable:
                                    ...
                                    site_tmp = get_matrix_column(
                                        record, num_samples, resolve_IUPAC
                                    )
                                    # Uncomment for debugging
                                    # print(site_tmp)
        ...
        print(
            "Genotypes excluded because they exceeded the amount "
            "of missing data allowed: {:d}".format(snp_shallow)
        )
        print(
            "Genotypes that passed missing data filter but were "
            "excluded for being MNPs: {:d}".format(mnp_num)
        )
        print("SNPs that passed the filters: {:d}".format(snp_accepted))
        if nexusbin:
            print(
                "Biallelic SNPs selected for binary NEXUS: {:d}".format(snp_biallelic)
            )
        ...
        if s != idx_outgroup:
            if fasta or nexus or not phylipdisable:
                with open(outfile + ".tmp") as tmp_seq:
                    ...
                    # Print current progress
                    print(
                        "Sample {:d} of {:d}, '{}', added to the nucleotide matrix(ces).".format(
            ...
            if nexusbin:
                with open(outfile + ".bin.tmp") as bin_tmp_seq:
                    ...
                    # Print current progress
                    print(
                        "Sample {:d} of {:d}, '{}', added to the binary matrix.".format(
                            s + 1, len(sample_names), sample_names[s]
                        )
                    )
