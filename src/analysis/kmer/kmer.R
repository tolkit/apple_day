...
png(
    filename = "./img/drMalDome58_kmer_spectrum.png", width = 8,
    height = 6,
    units = "in",
    res = 400
)
...
png(
    filename = "./img/sangers_newton_illumina.png", width = 8,
    height = 6,
    units = "in",
    res = 400
)
par(mar = c(5, 5, 4, 2))
read_plot_fastk_histex(
    "./data/sanger_newton_drMalDome6.R1.txt",
    range = 2:200, ylim = c(0, 8000000)
)
dev.off()

sanger_newton_ill_spec <- read_plot_fastk_histex(
    "./data/sanger_newton_drMalDome6.R1.txt",
    RETURN = TRUE
)

fwrite(
    x = sanger_newton_ill_spec[, .(index, count)],
...
# analyse with tetmer
spec <- Tetmer::read.spectrum(
    f =
        "./data/sanger_newton_drMalDome6.R1_spectrum.txt"
)
tetmer(spec)
