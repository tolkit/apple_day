...
gcs <- fread("./data/apple_genome_gc.tsv",
  col.names = c("assembly", "chromosome", "gc")
)
lens <- fread("./data/apple_genome_lengths.tsv",
  col.names = c("assembly", "chromosome", "length")
)
n50s <- fread("./data/apple_genome_n50.tsv",
  col.names = c("assembly", "n50")
)
...
# add colours
plot_cols <- data.table(
  chromosome = unique(gc_len[length > 3000000]$chromosome),
  col = c(
